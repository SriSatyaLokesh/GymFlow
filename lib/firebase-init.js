  ...
  return {
    ...
    auth: {
      ...
      async registerOwner({ gymName, name, email, password }) {
        ...
        const gymSettingsRef = firestoreApi.collection(db, "gym_settings");
        const gymQuery = firestoreApi.query(gymSettingsRef, firestoreApi.where("gymName", "==", trimmedGymName));
        const gymSnap = await firestoreApi.getDocs(gymQuery);
        if (!gymSnap.empty) {
