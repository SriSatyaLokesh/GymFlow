  ...
  const authService = {
    onAuthChange(callback) {
      ...
      async resetPassword(email) {
        let username = String(email || "").trim();
        const digitsOnly = username.replace(/\D/g, "");
        const isPhone = digitsOnly.length >= 10 && !username.includes("@");
        if (isPhone) {
          throw new Error(`Phone Login: Reset links cannot be emailed to synthetic phone accounts (+91 ${digitsOnly.slice(-10)}). Please ask your Gym Owner to reset your password from the Members roster.`);
        }
        try {
          await authApi.sendPasswordResetEmail(auth, username);
          return `Password reset email sent to ${username}. Check your inbox.`;
        } catch (error) {
          throw friendlyAuthError(error);
        }
      },
      ...
      async resetPassword(email) {
        ...
        const userObj = state.users.find(
          (user) => (user.email && user.email.toLowerCase() === normalized) ||
                    (isPhone && user.mobile && user.mobile.replace(/\D/g, "").slice(-10) === digitsOnly.slice(-10))
        );
        if (!userObj) {
          throw new Error(`No local account found for ${isPhone ? "phone +91 " + digitsOnly.slice(-10) : email}.`);
        }
        userObj.password = "password123";
        saveState();
        if (isPhone) {
          return `Password for phone +91 ${digitsOnly.slice(-10)} reset to "password123".`;
        }
        return `Password for ${email} reset to "password123".`;
      },
      async useDemo() {
